...
namespace ns3 {
  ...
  MyGymEnv::calculate_rewards() {
    ...
    for (itr = UserThrouput.begin(); itr != UserThrouput.end(); itr++) {
      ...
      avgUserThroughput+=all/m_UesNum.at(itr->first-1);
    ...
    //CALCULATING UNCONNECTED USERS WITH CQI < 1
    ...
    //ADDING PENALIZED THROPUGHPUT
    float lampda=2;
    rewards.at(1)=m_dlThroughput-avgUserThroughput*Dz_Users_num*lampda;
  }
  float
  MyGymEnv::GetReward() {
    NS_LOG_FUNCTION(this);
