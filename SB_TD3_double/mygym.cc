...
namespace ns3 {
  ...
  MyGymEnv::MyGymEnv(double stepTime, uint32_t N1, uint32_t N2, uint16_t N3,uint8_t ID, ns3::NetDeviceContainer cells) {
    NS_LOG_FUNCTION(this);
    Agent_ID=ID; //This tells the Env which agent / stage we are IN to calulate thier spicific reward 1 (first cluster) 2(second cluster) 3(Super Agent)
    m_cells =cells;
    collect = 0;
  ...
  MyGymEnv::ExecuteActions(Ptr < OpenGymDataContainer > action) {
    ...
    if(Agent_ID==1)
    {
      Current_list.at(0)=actionVectord.at(0);
      Current_list.at(2)=actionVectord.at(1);
      Current_list.at(3)=actionVectord.at(2);
        celloffset[5][5]=10;
    }
    if(Agent_ID==2)
    {
      Current_list.at(5)=actionVectord.at(0);
      Current_list.at(9)=actionVectord.at(1);
      Current_list.at(10)=actionVectord.at(2);
      celloffset[5][4]=6;
    }
    if(Agent_ID==3)
    {
      Current_list.at(1)=actionVectord.at(0);
      Current_list.at(4)=actionVectord.at(1);
      Current_list.at(6)=actionVectord.at(2);
      Current_list.at(7)=actionVectord.at(3);
      Current_list.at(8)=actionVectord.at(4);
      celloffset[4][4]=10;
    }
