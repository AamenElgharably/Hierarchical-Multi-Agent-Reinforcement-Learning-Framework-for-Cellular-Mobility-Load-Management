...
class multi_level_env(gym.Env):
    def __init__(self,Agent_ID,PRT):#Agent_ID should Match the one in the Simulator 
        ...
        self.Users=20
        if self.ID==3:
            self.state_dim=8+4+6
        else:
            self.state_dim = self.Cluster_num*4+5
    ...
    def reset(self):
        if self.ID==1:
            ...
            np.savez('Sch.npz',array1=np.array(self.ID+2))
            return self.get_state(state)
        else:
            x = [0 for i in range(self.action_dim)]
            np.savez('Sch.npz',array1=np.array(self.ID))
    ...
    def step(self,action):
        ...
        while (next_state == None): #wait for the other agent when it closes the env
            next_state, reward, done, info = self.env.step(action)
        if self.ID==3:
    ...
    def group_state(self,state):
        ...
        state=np.concatenate((state1_1,state1_2,state2_1,state2_2,state3_1,state3_2,state4_1,state4_2,state1[1],state2_norm[1],state3_norm[1],state4[1],state5_cluster_action),axis=None)
    ...
    def get_state(self,next_state):
        ...
        next_state  = np.concatenate((state1[self.Cluster],state2_norm[self.Cluster],state3_norm[self.Cluster],state4[self.Cluster],state5_intercluster_action),axis=None)  
